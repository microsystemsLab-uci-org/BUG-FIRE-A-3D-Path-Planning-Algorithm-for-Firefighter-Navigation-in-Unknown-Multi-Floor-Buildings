...
for i = 1:numSimulations
    ...
    if useParallel
        ...
        f = parfeval(@fcn_BUGFIRE, 5, startPoint, goalPoint, visionRadius, visionAngle);
        try
            [completedIdx, path, trajectory_length, goal3DReached, emergency_trajectory_length, bug2_trajectory_length] = fetchNext(f, timeoutDuration);
            executionTime = toc;
            ...
            if completedIdx > 0
                ...
                emergency_trajectory_length = NaN;
                bug2_trajectory_length = NaN;
        ...
        try
            [path, trajectory_length,goal3DReached, emergency_trajectory_length, bug2_trajectory_length] = fcn_BUGFIRE(startPoint, goalPoint, visionRadius, visionAngle);
            executionTime = toc;
    ...
    if ~exist('emergency_trajectory_length', 'var')
        emergency_trajectory_length = NaN;
    end
    if ~exist('bug2_trajectory_length', 'var')
        bug2_trajectory_length = NaN;
    end
    ...
    resultsTable.EmergencyTrajectoryLength(i) = emergency_trajectory_length;
    resultsTable.Bug2TrajectoryLength(i) = bug2_trajectory_length;
...
averageEmergencyTrajectoryLength = mean(resultsTable.EmergencyTrajectoryLength, 'omitnan');
averageBug2TrajectoryLength = mean(resultsTable.Bug2TrajectoryLength, 'omitnan');
...
disp(['Average Emergency exploration length: ', num2str(averageEmergencyTrajectoryLength), ' [m]']);
disp(['Average BUG2 path length: ', num2str(averageBug2TrajectoryLength), ' [m]']);
