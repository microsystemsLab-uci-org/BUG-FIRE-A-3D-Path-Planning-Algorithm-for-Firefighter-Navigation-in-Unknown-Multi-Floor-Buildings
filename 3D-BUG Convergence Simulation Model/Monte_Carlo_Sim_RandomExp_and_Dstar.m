...
visionRadius = 0.5; % Agent's vision radius in meters
...
resultsTable = table('Size', [numSimulations 8], ...
                     ...
                     'VariableNames', {'Iteration', 'StartPoint', 'GoalPoint', 'GoalReached', 'ExecutionTime', 'PathLength', 'RandomTrajectoryLength', 'DstarTrajectoryLength'});
...
for i = 1:numSimulations
    ...
    if useParallel
        ...
        f = parfeval(@fcn_RandomExp_and_Dstar, 5, startPoint, goalPoint, visionRadius, visionAngle);
        try
            [completedIdx, path, trajectory_length, goal3DReached, random_trajectory_length, dstar_trajectory_length] = fetchNext(f, timeoutDuration);
            executionTime = toc;
        ...
        try
            [path, trajectory_length,goal3DReached, random_trajectory_length, dstar_trajectory_length] = fcn_RandomExp_and_Dstar(startPoint, goalPoint, visionRadius, visionAngle);
            executionTime = toc;
    ...
    if ~exist('random_trajectory_length', 'var')
        random_trajectory_length = NaN;
    end
    ...
    resultsTable.RandomTrajectoryLength(i) = random_trajectory_length;
...
averageRandomTrajectoryLength = mean(resultsTable.RandomTrajectoryLength, 'omitnan');
...
disp(['Average Random exploration length: ', num2str(averageRandomTrajectoryLength), ' [m]']);
