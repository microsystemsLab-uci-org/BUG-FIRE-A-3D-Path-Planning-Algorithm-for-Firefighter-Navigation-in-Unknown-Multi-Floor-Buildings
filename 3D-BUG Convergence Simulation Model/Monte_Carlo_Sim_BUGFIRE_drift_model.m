...
%   performance of Eudald's BUGFIRE algorithm in a 3D environment, specifically
%   the Engineering Gateway building at the University of California, Irvine 
%   with IMU expected drift by ZUPT-aided INS.
%
...
useParallel = false; % Flag to enable/disable parallel execution
visionRadius = 1; % Agent's vision radius in meters
...
%                      'VariableNames', {'Iteration', 'StartPoint', 'GoalPoint', 'GoalReached', 'ExecutionTime', 'PathLength', 'EmergencyTrajectoryLength', 'Bug2TrajectoryLength'});

resultsTable = table('Size', [numSimulations 15], ...
                     'VariableTypes', {'double', 'string', 'string', ...
                                      'logical', 'logical', 'logical',...
                                      'double', 'double', 'double', 'double', ...
                                      'double', 'double', 'double', 'double', 'double'}, ...
                     'VariableNames', {'Iteration', 'StartPoint', 'GoalPoint', ...
                                      'GoalReached_True', 'GoalReached_Est', 'FinalPosWithin5m', ...
                                      'ExecutionTime', ...
                                      'PathLength_True', 'PathLength_Est', ...
                                      'EmergencyLength_True', 'EmergencyLength_Est', ...
                                      'Bug2Length_True', 'Bug2Length_Est', ...
                                      'DriftError_m', 'DriftError_Percent'});
...
% Adjust waitbar height to fit all text lines
waitbarFig = findobj(h, 'Type', 'Figure');
if ~isempty(waitbarFig)
    currentPos = get(waitbarFig, 'Position');
    % Increase height by ~50% to accommodate 3 lines of text
    newHeight = currentPos(4) * 1.5;
    set(waitbarFig, 'Position', [currentPos(1), currentPos(2) - (newHeight - currentPos(4))/2, currentPos(3), newHeight]);
end
...
for i = 1:numSimulations
    ...
    if i > 1
        currentSuccessRate_true  = (successCount / (i-1)) * 100;
        currentSuccessRate_est = (successCount_est / (i-1)) * 100;
    else
        currentSuccessRate_true = 0;
        currentSuccessRate_est = 0;
    end
    waitbar(i/numSimulations, h, sprintf('Running simulation %d of %d\nTrue Position Success Rate: %.2f%%\nDrifted Position Success Rate: %.2f%%', ...
        i, numSimulations, currentSuccessRate_true, currentSuccessRate_est));
    ...
    % Run the BUG-FIRE algorithm with timeout
    tic;
    if useParallel
        % Use parallel execution
        f = parfeval(@fcn_BUGFIRE_drift_model, 12, startPoint, goalPoint, visionRadius, visionAngle);
        try
            [completedIdx, path_true, path_est, trajectory_length_true, trajectory_length_est, ...
             goal3DReached_true, goal3DReached_est, emergency_trajectory_length_true, ...
             emergency_trajectory_length_est, bug2_trajectory_length_true, bug2_trajectory_length_est,...
             final_pos_true, final_pos_est] = fetchNext(f, timeoutDuration);
            executionTime = toc;
            
            if completedIdx > 0
                if goal3DReached_true
                    successCount = successCount + 1;
                    pathLengths(end+1) = trajectory_length_true;
                ...
                % Track estimated success/failure separately
                if goal3DReached_est
                    successCount_est = successCount_est + 1;
                else
                    failureCount_est = failureCount_est + 1;
                end
            else
                % Timeout occurred
                cancel(f);
                ...
                trajectory_length_true = NaN;
                trajectory_length_est = NaN;
                emergency_trajectory_length_true = NaN;
                emergency_trajectory_length_est = NaN;
                bug2_trajectory_length_true = NaN;
                bug2_trajectory_length_est = NaN;
                final_pos_true = [];  % **NEW**
                final_pos_est = [];   % **NEW**
        ...
        try
            [path_true, path_est, trajectory_length_true, trajectory_length_est, ...
             goal3DReached_true, goal3DReached_est, emergency_trajectory_length_true, ...
             emergency_trajectory_length_est, bug2_trajectory_length_true, bug2_trajectory_length_est,...
             final_pos_true, final_pos_est] = fcn_BUGFIRE_drift_model(startPoint, goalPoint, visionRadius, visionAngle);
            executionTime = toc;
    ...
    if ~exist('trajectory_length_true', 'var')
        trajectory_length_true = NaN;
        trajectory_length_est = NaN;
    end
    if ~exist('emergency_trajectory_length_true', 'var')
        emergency_trajectory_length_true = NaN;
        emergency_trajectory_length_est = NaN;
    end
    if ~exist('bug2_trajectory_length_true', 'var')
        bug2_trajectory_length_true = NaN;
        bug2_trajectory_length_est = NaN;
    end

    % Calculate drift error
    if ~isnan(trajectory_length_true) && ~isnan(trajectory_length_est)
        drift_error_m = abs(trajectory_length_est - trajectory_length_true);
        drift_error_percent = (drift_error_m / trajectory_length_true) * 100;
    else
        drift_error_m = NaN;
        drift_error_percent = NaN;
    end

    CELLS_PER_METER = 4;
    finalPosWithin5m = false;
    if ~isempty(final_pos_true) && ~isempty(final_pos_est)
        % Convert cells to meters and calculate distance
        final_distance_m = norm(final_pos_true - final_pos_est) / CELLS_PER_METER;
        finalPosWithin5m = final_distance_m < 5.0;
    end
    ...
    resultsTable.GoalReached_True(i) = goal3DReached_true;
    resultsTable.GoalReached_Est(i) = goal3DReached_est;
    resultsTable.FinalPosWithin5m(i) = finalPosWithin5m;
    resultsTable.ExecutionTime(i) = executionTime;
    resultsTable.PathLength_True(i) = trajectory_length_true;
    resultsTable.PathLength_Est(i) = trajectory_length_est;
    resultsTable.EmergencyLength_True(i) = emergency_trajectory_length_true;
    resultsTable.EmergencyLength_Est(i) = emergency_trajectory_length_est;
    resultsTable.Bug2Length_True(i) = bug2_trajectory_length_true;
    resultsTable.Bug2Length_Est(i) = bug2_trajectory_length_est;
    resultsTable.DriftError_m(i) = drift_error_m;
    resultsTable.DriftError_Percent(i) = drift_error_percent;

    fprintf('Simulation %d/%d completed.\n', i, numSimulations);
    fprintf('  True  - Success: %d, Failure: %d (%.2f%% success)\n', ...
        successCount, failureCount, (successCount / i) * 100);
    fprintf('  Drift - Success: %d, Failure: %d (%.2f%% success)\n', ...
        successCount_est, failureCount_est, (successCount_est / i) * 100);
end
...
averagePathLength_True = mean(resultsTable.PathLength_True, 'omitnan');
averagePathLength_Est = mean(resultsTable.PathLength_Est, 'omitnan');
averageExecutionTime = mean(resultsTable.ExecutionTime);
averageEmergencyLength_True = mean(resultsTable.EmergencyLength_True, 'omitnan');
averageEmergencyLength_Est = mean(resultsTable.EmergencyLength_Est, 'omitnan');
averageBug2Length_True = mean(resultsTable.Bug2Length_True, 'omitnan');
averageBug2Length_Est = mean(resultsTable.Bug2Length_Est, 'omitnan');
averageDriftError = mean(resultsTable.DriftError_m, 'omitnan');
averageDriftErrorPercent = mean(resultsTable.DriftError_Percent, 'omitnan');

disp('------------------------------------------------------------------------');
disp('Monte Carlo Simulation Results:');
disp(['Number of simulations: ', num2str(numSimulations)]);
disp(['Success rate (True): ', num2str(successRate), '%']);
disp(['Success rate (Est): ', num2str(sum(resultsTable.GoalReached_Est)/numSimulations*100), '%']);
disp(['Final positions within 5m: ', num2str(sum(resultsTable.FinalPosWithin5m)/numSimulations*100), '%']);
disp(' ');
disp('TRUE TRAJECTORIES:');
disp(['  Average total path length: ', num2str(averagePathLength_True), ' [m]']);
disp(['  Average Emergency exploration length: ', num2str(averageEmergencyLength_True), ' [m]']);
disp(['  Average BUG2 path length: ', num2str(averageBug2Length_True), ' [m]']);
disp(' ');
disp('ESTIMATED (DRIFTED) TRAJECTORIES:');
disp(['  Average total path length: ', num2str(averagePathLength_Est), ' [m]']);
disp(['  Average Emergency exploration length: ', num2str(averageEmergencyLength_Est), ' [m]']);
disp(['  Average BUG2 path length: ', num2str(averageBug2Length_Est), ' [m]']);
disp(' ');
disp('DRIFT ANALYSIS:');
disp(['  Average drift error: ', num2str(averageDriftError), ' [m]']);
disp(['  Average drift error percentage: ', num2str(averageDriftErrorPercent), ' [%]']);
