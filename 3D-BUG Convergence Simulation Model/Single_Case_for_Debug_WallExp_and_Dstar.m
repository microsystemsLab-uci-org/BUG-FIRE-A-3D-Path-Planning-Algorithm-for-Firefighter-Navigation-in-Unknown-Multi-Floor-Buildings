...
switch scenario
    ...
    case 18
        startHeight = 0; 
        startPoint  = [108, 89, startHeight];  
        goalHeight = 200;               
        goalPoint  = [50, 165, goalHeight];
    case 20
        startHeight = 0; 
        startPoint  = [87, 36, startHeight];  
        goalHeight = 100;               
        goalPoint  = [84, 195, goalHeight];
    case 21
        startHeight = 0; 
    ...
    case 22
        startHeight = 100; 
        startPoint  = [87, 317, startHeight];  
        goalHeight = 200;               
        goalPoint  = [85, 70, goalHeight];
    case 23
        startHeight = 0; 
        startPoint  = [90, 114, startHeight];  
        goalHeight = 100;               
        goalPoint  = [87, 278, goalHeight];
...
while true
    ...
    if startHeight == goalHeight
        ...
        total_path = vertcat(total_path, horzcat(path, ones(length(path),1)*goalHeight));
        diff_path = diff(path);
...
% wall Path Summary
if wall_path_length_cells > 0
    fprintf('🔍 WALL EXPLORATION PHASE:\n');
    fprintf('   • Path length (cells):  %d cells\n', wall_path_length_cells);
    fprintf('   • Path length (meters): %.2f m\n', wall_path_length_meters);
    fprintf('   • Average step size:    %.3f m/cell\n', wall_path_length_meters/wall_path_length_cells);
    fprintf('\n');
else
    fprintf('🔍 WALL EXPLORATION PHASE: Not executed\n\n');
end
...
if wall_path_length_cells > 0 && dstar_path_length_cells > 0
    wall_percentage = (wall_path_length_meters / total_path_length_meters) * 100;
    dstar_percentage = (dstar_path_length_meters / total_path_length_meters) * 100;
    
    fprintf('📈 PERFORMANCE BREAKDOWN:\n');
    fprintf('   • wall exploration:    %.1f%% of total distance\n', wall_percentage);
